...
# Rematch stages: id suffix -> level bump
STAGES = {
    2: 5,
    3: 10,
    4: 15,
...
# Evolve map for rematches (species substring replacements, first match)
EVOLUTIONS = [
    ("Glimmet", "Glimmora"),
    ("Nosepass", "Probopass"),
    ("Aron", "Lairon"),
...
def bump_party(block: str, base_id: str, stage: int, bump: int) -> str:
    out_id = base_id[:-1] + str(stage) if base_id.endswith("_1") else f"{base_id}_{stage}"
    # Handle TRAINER_ROXANNE_1 -> TRAINER_ROXANNE_2
    if base_id.endswith("_1"):
        out_id = base_id[:-1] + str(stage)
    else:
        out_id = f"{base_id}_{stage}"
    ...
    while i < len(lines):
        line = lines[i]
        # Species line (not Level/Ability/etc and not starting with -)
        if (
            i > 0
            and line
            ...
            # Possibly evolve on stage >= 3
            species_line = line
            if stage >= 3:
                for pre, post in EVOLUTIONS:
                    if species_line.startswith(pre + " ") or species_line == pre or species_line.startswith(pre + " @"):
                        species_line = species_line.replace(pre, post, 1)
                        # Drop Eviolite on evolved
...
def main():
    ...
    for base in bases:
        ...
        for stage, bump in STAGES.items():
            ...
            # Ensure doubles for Norman/Tate rematches
            if "NORMAN" in rematch_id or "TATE" in rematch_id:
                new_block = new_block.replace("Battle Type: Singles", "Battle Type: Doubles")
            ...
            if m:
                ...
                # Insert before TRAINER_WATTSON_3 or at end of file before last trainers - append after story juan block's rematch area
                # Prefer insert near other rematch trainers: before TRAINER_WATTSON_3 if creating WATTSON_2 etc.
