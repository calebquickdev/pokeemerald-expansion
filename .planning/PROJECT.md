...
- [ ] "Don't Invite Others" begins raid battle with 2 CPU allies

**Raid Battle**
- [ ] 3v1 layout: player's chosen Pokémon + 2 CPU allies vs. permanently-Dynamaxed boss (4 battlers total; engine max is 4)
- [ ] Boss level randomized within range by star rating (1★: 15–20, 2★: 25–30, 3★: 35–40, 4★: 45–50, 5★: 55–60)
- [ ] Boss HP multiplied by 3
...
- [ ] Dynamax rotation: turn 1 = player can Dynamax, turn 2 = CPU ally 1, turn 3 = CPU ally 2, turn 4 = player again (cycles)
...
- [ ] Left side UI: animated icon sprites of 3 allied Pokémon (player + 2 CPU), each with a simple relative HP bar
...
**CPU Allies**
- [ ] 2 CPU allies selected randomly from a hardcoded placeholder species list (to be expanded later)
- [ ] 2 CPU allies use the same AI controller as standard opponent battlers
...
- **Battler count**: Engine max is 4 battlers; raid is 3v1 (player + 2 CPU allies + boss)
...
| Species pool uses base-form species only; random form selected within species | Prevents duplicate/redundant entries in the pool | — Pending |
| GMAX catch delivers base form with Gigantamax Factor | Matches mainline Sword/Shield behavior | — Pending |
| GMAX Pokémon are the only exception to the base-form-only pool rule | They spawn and display as GMAX form but are caught as base + factor | — Pending |
| Shields trigger at 75% and 50% HP (two separate events) | Confirmed by owner; matches mainline behavior | — Pending |
| 3v1 layout (player + 2 CPU allies + boss = 4 battlers) | Engine MAX_BATTLERS_COUNT = 4; extending is risky refactor | — Pending |
| Boss HP × 3 (reduced from × 4 to match 3-attacker balance) | Adjusted when ally count reduced from 3 to 2 | — Pending |
...
---
*Last updated: 2026-02-27 after research (ally count: 3→2, boss HP: ×4→×3)*
